    ...
    $.extend(CORE_TEMP.function, {
        SiteColor: function () {
            var $site_color = $('.site-color'), /* variable used on Site Color Settings */
                ...
                $cssRoot = '../../assets/layouts/layout_left_menu_angular/css/color/'; /* variable used on Site CSS path */
            ...
            $nav_overlay.on('click', function (e) {
                ...
                if ($(".nav-fixed-check").prop("checked") == true) {
                    ...
                    if($body.hasClass('nav-menu-icon')){
                        CORE_TEMP.function.initPerfectScroll($(".scroller"));
                    }
                    $(".main-menu").children(".sub-item.open").children('.sub-menu').css("display","block");
                } else {
                    $('.nav-fixed-text').css("font-weight", "normal").html("fixed");
                    localStorage.removeItem("navigation_type");
                    $body.removeClass('menu-overlay');
                    if($body.hasClass('nav-menu-icon')){
                        CORE_TEMP.function.DestroyPerfectScroll($(".scroller"));
                    }
                }
        ...
        LeftMenuTogglebutton: function () {
            ...
            $('.left-menu-toggle').on('click', function (e) {
                ...
                if (!$body.hasClass('nav-menu-icon') && !$body.hasClass('nav-menu-hide')) {
                    ...
                    if ($(window).width() < 768) {
                        $body.addClass('nav-menu-hide').removeClass('nav-menu-clicked');
                    }
                    else {
                        ...
                        if (!$body.hasClass('menu-overlay')) {
                            CORE_TEMP.function.DestroyPerfectScroll($(".scroller"));
                            $(".main-menu").children(".sub-item.open").addClass("active").children('.sub-menu').removeAttr("style");
                        }
                    }
                }
        ...
        Leftsidemenu: function () {
            $('.sidebar-menu .sub-item > a').on('click', function () {
                ...
                if (!$body.hasClass('nav-menu-icon') || $body.hasClass('menu-overlay')) {
                    var $this = $(this).parent('li');
                    if($this.children('ul').hasClass('sub-menu') && !$this.parents('ul').hasClass('sub-menu')){
                        $('ul.sub-menu').not($this.children('.sub-menu')).slideUp();
                        $('.sidebar-menu li').not('.menu-title').not($this).removeClass('active');
        ...
        NavigetionMenuActive: function () {
            if (!$body.hasClass('nav-menu-icon')) {
                var pageurl = location.hash.toLowerCase(),
                    menuContainer = $('#site-menu');
                menuContainer.find('li.active').removeClass('active open');
                menuContainer.find("li > a").each(function() {
                    var path = $(this).attr("href").toLowerCase();
                    // url match condition
                    if (path.length > 1 && pageurl.substr(1, path.length - 1) == path.substr(1)) {
                ...
                // $('#live-search-box').val('');
                // $('.menu-title').show();
                // $('.live-search-list').find("li a").show();
            }
        },
